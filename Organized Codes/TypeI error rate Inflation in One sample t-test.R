#TypeI error rate Inflation in One sample t-test
...
row.names(altmatrix) <- row.names(power_normt_matrix) <- row.names(normmatrix) <- smpn
colnames(altmatrix) <- colnames(power_normt_matrix) <- colnames(normmatrix) <- dfvec
##########################################################################################
##########################################################################################
...
for(i in 1:length(smpn)){
  ...
  for(k in 1:length(dfvec)){
    ...
    altmatrix[i,k] <- mean(rejectH0_alt)
...
plot(smpn,normmatrix[,1], 
     ...
     main = 'df=0.5')
lines(smpn, altmatrix[,1], col = "red")
#df=1
plot(smpn,normmatrix[,2], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=1')
lines(smpn, altmatrix[,2], col = "red")
#df=1.5
plot(smpn,normmatrix[,3], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=1.5')
lines(smpn, altmatrix[,3], col = "red")
#df=2
plot(smpn,normmatrix[,4], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=2')
lines(smpn, altmatrix[,4], col = "red")
#df=5
plot(smpn,normmatrix[,5], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=5')
lines(smpn, altmatrix[,5], col = "red")
#df=10
plot(smpn,normmatrix[,6], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=10')
lines(smpn, altmatrix[,6], col = "red")
#df=15
plot(smpn,normmatrix[,7], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=15')
lines(smpn, altmatrix[,7], col = "red")
#df=30
plot(smpn,normmatrix[,8], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'df=30')
lines(smpn, altmatrix[,8], col = "red")

#Organized plot for df in [0.5,5]
plot(smpn,normmatrix[,1], 
     type = "l", ylim = c(0, 0.2),
     ylab = 'Type I error rate',
     main = 'Comparison under different df')
lines(smpn, altmatrix[,1], col = "red", lty = 5)
lines(smpn, altmatrix[,2], col = "green",lty=6)
lines(smpn, altmatrix[,3], col = "blue",lty=7)
lines(smpn, altmatrix[,4], col = "gold",lty=8)
lines(smpn, altmatrix[,5], col = "purple",lty=9)
legend("topright", legend=c("df=0.5", "df=1", 'df=1.5','df=2','df=5'),
       col=c("red", "green",'blue','gold','purple'), lty = c(5:9), cex=0.8)
###Observation: 
...
#df=0.5
plot(power_normt_matrix[,1], altmatrix[,1]-normmatrix[,1],
      pch = 15, ylim = c(0, 0.3),col = 'red',
     main="Normality test power vs TypeI error inflation")
#df=1
points(power_normt_matrix[,2], altmatrix[,2]-normmatrix[,2],
     pch = 16, ylim = c(0, 0.3), col = 'green')
#df=1.5
points(power_normt_matrix[,3], altmatrix[,3]-normmatrix[,3],
     pch = 17, ylim = c(0, 0.3),col='blue')
#df=2
points(power_normt_matrix[,4], altmatrix[,4]-normmatrix[,4],
     pch = 18, ylim = c(0, 0.3),col='gold')
#df=5
points(power_normt_matrix[,5], altmatrix[,5]-normmatrix[,5],
...
save.image(paste0("TypeI-One sample t-test",".RData"))
