...
for (s in 1:length(dvec)){
  ...
  for(i in 1:length(smpn)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        x <- rbeta(n,shape1 = df,shape2 = df/2)#mean=df/(df+df/2)=2/3
        y <- (x - 2/3)/sqrt(4/(9*(3*df+2)))##############var=(df*df/2)/(df+df/2)^2*(df+df/2+1)
        z <- y + d
        
        x1 <- rbeta(n,shape1 = df,shape2 = df/2)#mean=df/(df+df/2)=2/3
        y1 <- (x1 - 2/3)/sqrt(4/(9*(3*df+2)))##############var=(df*df/2)/(df+df/2)^2*(df+df/2+1)
        z1 <- y1 + d
...
save.image(paste0("Twosampt-Beta-TypeI-Power",".RData"))

#################################################################################
#################################################################################
library(Cairo)
#TypeI Error
Cairo(file = "betaplot.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
#Adverse effect in downstream test
plot(smpn, altmatrixc[1,,1], typ = "l", ylim = c(0.02, 0.08),
     xlab = "Sample size", ylab = "P(Type-I Error)", col = "red",
     main = "Adverse effect in downstream test")
for (j in c(2,4,5)){
  lines(smpn, altmatrixc[1,,j], col = j+1)
}
abline(h = 0.05, lty = 2, lwd = 0.6)
legend("topright", 
       legend = c("Beta(4,2)", "Beta(6,3)", "Beta(10,5)", "Beta(20,10)"), 
       lty = 1, col = c(2,3,5,6), cex = 0.7)

#Utility of normality test
j <- 1
type1inf <- altmatrixc[1,,j] - 0.05
plot(power_normt_matrix[1,,j], type1inf, typ = "l", col = "red",
     ylab = "Inflation of Type-I error", 
     xlab = "Power of normality test",
     ylim = c(-0.01, 0.05), xlim = c(0.05, 0.8),
     main = "Utility of normality test")
for (j in c(2, 4, 5)){
  type1inf <- altmatrixc[1,,j] - 0.05
  lines(power_normt_matrix[1,,j], type1inf, typ = "l", col = j+1,
        ylab = "Inflation of Type-I error")
}
...
################################################################################
#Power-d=0.5
Cairo(file = "beta-power-0.5.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
######d=0.5
#Adverse effect in downstream test
...
legend("topright", legend = c("Beta(4,2)", "Beta(6,3)", "Beta(10,5)", "Beta(20,10)", "Normal"), 
       lty = c(1,1,1,1,2), col = c(2,3,5,6,7), lwd = c(1,1,1,1,2),cex = 0.7)

#Utility of normality test
j <- 1
powerdif <- normmatrix[2,,j] - altmatrixc[2,,j]
plot(power_normt_matrix[2,,j], powerdif, typ = "l", col = "red",
     ylab = "Power Loss", 
     xlab = "Power of normality test",
...
for (j in c(2,4,5)){
  powerdif <- normmatrix[2,,j] - altmatrixc[2,,j]
  lines(power_normt_matrix[2,,j], powerdif, typ = "l", col = j+1)
}
abline(h = 0, lty = 2, lwd = 0.6)
legend("bottomleft", legend = c("Beta(4,2)", "Beta(6,3)", "Beta(10,5)", "Beta(20,10)"), 
       lty = 1, col = c(2,3,5,6), cex = 0.7)
dev.off()
...
#################################################################################
#Power-d=1
Cairo(file = "beta-power-1.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
######d=1
#Adverse effect in downstream test
...
legend("topright", legend = c("Beta(4,2)", "Beta(6,3)", "Beta(10,5)", "Beta(20,10)", "normal"), 
       lty = c(1,1,1,1,2), col = c(2,3,5,6,7), lwd = c(1,1,1,1,2),
       cex = 0.7)

#Utility of normality test
j <- 1
powerdif <- normmatrix[3,,j] - altmatrixc[3,,j]
plot(power_normt_matrix[3,,j], powerdif, typ = "l", col = "red",
     ylab = "Power Loss", 
     xlab = "Power of normality test",
     ylim = c(-0.3, 0.10), xlim = c(0.05, 1),
     main = "Utility of normality test")
for (j in c(2,4,5)){
  powerdif <- normmatrix[3,,j] - altmatrixc[3,,j]
  lines(power_normt_matrix[3,,j], powerdif, typ = "l", col = j+1)
}
abline(h = 0, lty = 2, lwd = 0.6)
legend("bottomleft", legend = c("Beta(4,2)", "Beta(6,3)", "Beta(10,5)", "Beta(20,10)"), 
       lty = 1, col = c(2,3,5,6), cex = 0.7)
