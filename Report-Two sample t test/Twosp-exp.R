...
for (s in 1:length(dvec)){
  ...
  for(i in 1:length(smpn)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        x <- generate_data(n, "Expotential", df) #rate = 1/df, so mean = df, sd = df
        y <- (x-df)/df
        z <- y + d
        
        x1 <- generate_data(n, "Expotential", df) #rate = 1/df, so mean = df, sd = df
        y1 <- (x1-df)/df
        z1 <- y1 + d
...
save.image(paste0("twosampt-exp-TypeI-Power",".RData"))

###################################################################################
################################################################################
library(Cairo)
#TypeI Error Rate
Cairo(file = "exp-TypeIErr.pdf",typ = "pdf",dpi=95,
...
legend("topright", legend = paste("1/rate = ", dfvec[c(1:5)]), 
...
#################################################################################
#Power-d=0.5
Cairo(file = "exp-power-d=0.5.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
######d=0.5
#Adverse effect in downstream test
...
legend("topright", legend = c(paste("1/rate = ", dfvec[c(1:5)]), "normal"), 
...
abline(h = 0, lty = 2, lwd = 0.6)
legend("bottomleft", legend = paste("1/rate = ", dfvec[c(1:5)]), 
...
#################################################################################
#Power-d=1
Cairo(file = "exp-power-1.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
######d=1
#Adverse effect in downstream test
