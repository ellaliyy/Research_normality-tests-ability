  ...
  for(i in 1:length(smpn)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        ...
        x1 <- generate_data(n,"Chi-square", df)
        y1 <- (x1-df)/sqrt(2*df) 
        z1 <- y1 + d
...
save.image(paste0("Twosampt-Chisq--TypeI-Power",".RData"))

############################################################################
############################################################################
library(Cairo)
#TypeI Error Rate
Cairo(file = "Twosp-chiplot-TypeIErr.pdf",typ = "pdf",dpi=95,
...
legend("topright", legend = paste("df = ", dfvec[c(1,3,5,7)]), 
       lty = 1, col = c(2,3,5,7), cex = 0.7)
...
lines(smpn, apply(normmatrix[2,,], 1, mean), lty = 2, col = 8, lwd = 2)
legend("topright", legend = c(paste("df = ", dfvec[c(1:7)]), "normal"), 
       lty = c(1,1,1,1,1,1,1,2), col = c(1:8), lwd = c(1,1,1,1,1,1,1,2),
       cex = 0.7)

#Utility of normality test
j <- 1
powerdif <- normmatrix[2,,j] - altmatrixc[2,,j]
plot(power_normt_matrix[2,,j], powerdif, typ = "l", col = "1",
     ylab = "Power Loss", 
     xlab = "Power of normality test",
...
abline(h = 0, lty = 2, lwd = 0.6)
legend("bottomleft", legend = paste("df = ", dfvec[c(1:7)]), 
       lty = 1, col = c(1:7), cex = 0.5)
dev.off()

#################################################################################
#Power-d=1
Cairo(file = "chiplot-power-1.pdf",typ = "pdf",dpi=95,
      height = 500, width = 1100)
par(mfrow = c(1,2))
######d=1
#Adverse effect in downstream test
...
lines(smpn, apply(normmatrix[3,,], 1, mean), lty = 2, col = 8, lwd = 2)
legend("topright", legend = c(paste("df = ", dfvec[c(1:7)]), "normal"), 
       lty = c(1,1,1,1,1,1,1,2), col = c(1:8), lwd = c(1,1,1,1,1,1,1,2),
       cex = 0.7)
...
plot(power_normt_matrix[3,,j], powerdif, typ = "l", col = "1",
     ylab = "Power Loss", 
     xlab = "Power of normality test",
...
for (j in c(2:7)){
  powerdif <- normmatrix[3,,j] - altmatrixc[3,,j]
  lines(power_normt_matrix[3,,j], powerdif, typ = "l", col = j)
}
abline(h = 0, lty = 2, lwd = 0.6)
legend("bottomleft", legend = paste("df = ", dfvec[c(1:7)]), 
       lty = 1, col = c(1:7), cex = 0.5)
