  ...
  for(i in 1:length(nvec)){
    ...
    for(k in 1:length(varvec)){
      ...
      for(j in 1:N){
        ...
        #e ~ non-Normal
        e1 <- generate_data(n, "Expotential", sqrt(v))
        #rate = 1/sqrt(v), so mean = sqrt(v), sd = sqrt(v)
        e <- e1 - sqrt(v)
        y <- b0+b1*x+e
        out <- summary(lm(formula = y~x))
        pval <- out$coefficients[,4][2]
...
plot(nvec,altmatrixc[1,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "Power", xlab = "Sample Size", col = 1,
     main = "Exponential, Variance = 2, power ~ b1")
for (j in c(2:5)){
...
plot(nvec,altmatrixc[6,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "TypeI error rate", xlab = "Sample Size", col = 1,
     main = "Exponential, Variance = 2, TypeI error rate")
abline(h = 0.05, col = "red")
...
save.image(paste0("SLR-Exponential",".RData"))
