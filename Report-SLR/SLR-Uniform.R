  ...
  for(i in 1:length(nvec)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        ...
        #e ~ Normal
        e <- rnorm(n, mean = 0, sd= sqrt((df)^2/12))
        y <- b0+b1*x+e
        ...
        #e ~ non-Normal
        e1 <- runif(n,min = 0 , max = df)##mean=df/2, var=(df)^2/12
        e <- (e1 - df/2)
        y <- b0+b1*x+e
        out <- summary(lm(formula = y~x))
        pval <- out$coefficients[,4][2]
...
plot(nvec,altmatrixc[6,,1], 
     type = "l", ylim = c(0, 0.1), 
     ylab = "TypeI error rate", xlab = "Sample Size", col = 1,
     main = "Uniform, df = 4, TypeI error rate")
abline(h = 0.05, col = "red")
...
save.image(paste0("SLR-Uniform",".RData"))
