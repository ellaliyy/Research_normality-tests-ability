...
b0 <- 1
b1vec <- c(0.2, 0.5, 0.7, 1.0, 1.5,0)
#change b1 values to find good power, add 0 for typeI error rate
  ...
  for(i in 1:length(nvec)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        ...
        #e ~ Normal
        e <- rnorm(n, mean = 0, sd= sqrt(2*d))#var=2*d,mean=0
        y <- b0+b1*x+e
        ...
        #e ~ non-Normal
        e <- rchisq(n, df = d)- d #var=2d, so df=d=mean(integers)
        y <- b0+b1*x+e
        out <- summary(lm(formula = y~x))
        pval <- out$coefficients[,4][2]
        if(pval < alpha){
...
save.image(paste0("SLR_revised",".RData"))
#####################################################################
normmatrix
altmatrixc
library(Cairo)
...
#Normal--b1=0.2, df changes
plot(nvec,normmatrix[1,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "Power", xlab = "Sample Size", 
     main = "Normal, b1=0.2, power ~ variance/df")
for (j in c(2:5)){
...
plot(nvec,altmatrixc[1,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "Power", xlab = "Sample Size", 
     main = "Chi-sq, b1=0.2, power ~ variance/df")
for (j in c(2:5)){
...
plot(nvec,power_normt_matrix[1,,1], 
     ...
     main = "Normal, b1=0.2, power of Normality test ~ variance/df")
for (j in c(2:5)){
...
legend("bottomleft", legend = paste("df = ", dfvec),
       col=c(1:5),cex = 0.7, lty = 1)
...
save.image(paste0("SLR-Chisq-Revised",".RData"))
