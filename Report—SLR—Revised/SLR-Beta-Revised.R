...
b0 <- 1
b1vec <- c(2, 5, 7, 10, 15,0)#change b1 values to find good power, add 0 for typeI error rate
#b1 10x bigger than before

N <- 1000
alpha <- 0.05
nvec <- c(10,20,30,50,100)
dfvec <- c(4,6,10,20,30)
  ...
  for(i in 1:length(nvec)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        ...
        #e ~ Normal
        e <- rnorm(n, mean = 0, sd= sqrt((df*df/2)/((df+df/2)^2*(df+df/2+1))))
        y <- b0+b1*x+e
        ...
        #e ~ non-Normal
        e1 <- rbeta(n,shape1 = df,shape2 = df/2)#mean=df/(df+df/2)=2/3
        e <- (e1 - 2/3)########var=(df*df/2)/((df+df/2)^2*(df+df/2+1))
        y <- b0+b1*x+e
        out <- summary(lm(formula = y~x))
        pval <- out$coefficients[,4][2]
        if(pval < alpha){
...
plot(nvec,normmatrix[6,,1], 
     ...
     main = "Normal, df = 4, TypeI error rate")
...
plot(nvec,altmatrixc[6,,1], 
     type = "l", ylim = c(0, 0.1), 
     ylab = "TypeI error rate", xlab = "Sample Size", col = 1,
     main = "Beta, df = 4, TypeI error rate")
abline(h = 0.05, col = "red")
...
plot(nvec,power_normt_matrix[1,,1], 
     ...
     main = "Normal, b1=0.2, power of Normality test ~ df")
for (j in c(2:5)){
  lines(nvec, power_normt_matrix[1,,j], col = j)
}
legend("topleft", legend = paste("df = ", dfvec),
       col=c(1:5),cex = 0.7, lty = 1)
...
save.image(paste0("SLR-beta-Revised",".RData"))



#FEEDBACK:
#After change the b1 values 10x bigger and re-run, the plots are basicly the same as before.
