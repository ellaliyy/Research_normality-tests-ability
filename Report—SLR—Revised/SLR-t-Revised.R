  ...
  for(i in 1:length(nvec)){
    ...
    for(k in 1:length(dfvec)){
      ...
      for(j in 1:N){
        ...
        #e ~ Normal
        e <- rnorm(n, mean = 0, sd= sqrt(df/(df-2)))#var=v,mean=0
        y <- b0+b1*x+e
...
save.image(paste0("SLR_t_revised",".RData"))
#####################################################################
...
plot(nvec,altmatrixc[1,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "Power", xlab = "Sample Size", col = 1,
     main = "Student's  t, df = 3, power ~ b1")
for (j in c(2:5)){
...
plot(nvec,normmatrix[6,,1], 
     ...
     main = "Normal, df = 3, TypeI error rate")
...
plot(nvec,altmatrixc[6,,1], 
     type = "l", ylim = c(0, 0.2), 
     ylab = "TypeI error rate", xlab = "Sample Size", col = 1,
     main = "Student's t, df = 3, TypeI error rate")
abline(h = 0.05, col = "red")
...
plot(nvec,altmatrixc[1,,1], 
     type = "l", ylim = c(0, 1), 
     ylab = "Power", xlab = "Sample Size", 
     main = "Student's t, b1=0.2, power ~ df")
for (j in c(2:5)){
...
save.image(paste0("SLR-t-Revised",".RData"))




#FEEDBACK:
#after changing the df values smaller, didnot observe bigger power loss values 
#or bigger inflated typeI error rate.
